...
from .models import RentMonth, Transportation, Market, Salaries, Restaurant
...
def bsoup():
    ...
    for web in links:
        ...
        Average_Monthly_Net_Salary_price = Average_Monthly_Net_Salary_price.replace(' R$', '').replace(',', '').replace(
            '?', '0.00')

        try:
            restaurant = Restaurant.objects.get(Meal=meal_price, McDonalds=mcdonalds_price, CokePepsi=cokepepsi_price,
                                                Water=water_price)
            restaurant_id = restaurant.id
        except:
            restaurant = Restaurant.objects.create(Meal=meal_price, McDonalds=mcdonalds_price,
                                                   CokePepsi=cokepepsi_price, Water=water_price)
            restaurant_id = restaurant.id
        ...
        try:
            ...
            market = Market.objects.create(Milk=milk_price, Rice=rice_price, Eggs=eggs_price,
                                           Beef_Round=beef_Round_price)
            market_id = market.id

        try:
            transportation = Transportation.objects.get(One_way_Ticket=one_way_Ticket_price, Taxi_1km=taxi_1km_price,
                                                        Gasoline=gasoline_price)
            transportation_id = transportation.id
        except:
            transportation = Transportation.objects.create(One_way_Ticket=one_way_Ticket_price, Taxi_1km=taxi_1km_price,
                                                           Gasoline=gasoline_price)
            transportation_id = transportation.id

        try:
            rentmonth = RentMonth.objects.get(in_City_Centre=in_City_Centre_price,
                                              Outside_of_Centre=Outside_of_Centre_price)
            restaurant_id = rentmonth.id
        except:
            rentmonth = RentMonth.objects.create(in_City_Centre=in_City_Centre_price,
                                                 Outside_of_Centre=Outside_of_Centre_price)
